У вас есть массив целых чисел `nums` и целое число `k`. Определите есть ли два отличных индекса `i` и `j` в массиве, где `nums[i] = nums[j]` и разница между `i` и `j` меньше или равна `k`. 
